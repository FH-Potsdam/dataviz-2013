...
//        if (chart.rawMigrationData_men[i][3] == "w") { s = (PI/2) / 8;}
    ...
    chart.outerArcGenerator =
            d3.svg.arc().
            innerRadius(function(d, i) { return (chart.immigrationValuesCurrent[i] / 100) }).
    ...
    chart.countryLabels.enter().append("text").
            attr("x", function(d, i) { return (Math.cos(-Math.PI/2 + i*chart.stepSize - chart.stepSize/2)*400) } ).
            attr("y", function(d, i) { return (Math.sin(-Math.PI/2 + i*chart.stepSize - chart.stepSize/2)*400) } ).
            text(function(d, i) { return chart.countries[i].name });
